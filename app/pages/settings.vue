...
<template>
  <div class="flex min-h-[calc(100vh-64px)] w-full relative overflow-hidden">
    ...
    <!-- Sidebar -->
    <aside 
      class="fixed inset-y-0 left-0 z-40 bg-base-200 border-base-300 flex flex-col pt-8 transition-all duration-300 ease-in-out lg:static lg:z-auto shrink-0 overflow-hidden"
      :class="[
        isOpen 
    ...
    >
      <div class="w-64 h-full flex flex-col shrink-0 overflow-hidden">
        <div class="px-6 mb-6">
          <NuxtLink to="/" class="flex items-center gap-2 text-sm text-base-content/70 hover:text-base-content transition-colors w-fit group">
        ...
        <div class="px-6 mb-6 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="p-1.5 bg-base-300 rounded-md">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-base-content/80">
          ...
          <button 
            type="button" 
            class="lg:hidden text-base-content/70 hover:text-base-content p-1 rounded-md hover:bg-base-300 transition-colors"
            @click="close"
        ...
        <nav class="flex flex-col gap-1 px-3">
          <NuxtLink
            ...
            :class="route.path.startsWith(item.path) ? 'bg-primary text-primary-content shadow-sm' : 'text-base-content/70 hover:text-base-content hover:bg-base-300'"
    ...
    <!-- Main Content -->
    <main class="flex-1 bg-base-100 min-w-0 text-base-content">
      <NuxtPage />
    </main>
  </div>
</template>
