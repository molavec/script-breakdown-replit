    ...
    <main class="flex-1 flex overflow-hidden relative">
      ...
      <div class="flex-1 overflow-y-auto bg-base-100 text-base-content p-6 md:p-10">
        <div class="max-w-4xl mx-auto space-y-8 pb-16">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="breadcrumbs text-xs font-mono text-base-content/60 p-0">
                <ul>
                  ...
                  <li><NuxtLink :to="`/projects/${projectId}`" class="hover:text-primary transition-colors">{{ project?.name }}</NuxtLink></li>
                  <li class="text-base-content/50">Columns</li>
                  <li class="text-base-content font-semibold">{{ column ? column.name : 'Not Found' }}</li>
                </ul>
              </div>
            </div>
          ...
          <!-- Not Found State -->
          <div v-if="!column" class="card bg-base-200 border border-base-300 rounded-box p-12 text-center space-y-4 shadow-sm">
            ...
            <h2 class="text-xl font-bold text-base-content">Column not found</h2>
            <p class="text-sm text-base-content/60">The column identifier "{{ columnId }}" does not exist in this project.</p>
          ...
          <div v-else class="space-y-8">
            ...
            <div class="card bg-base-200 border border-base-300 rounded-box p-6 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
              <div class="flex items-center gap-4">
                <div 
                  class="w-12 h-12 rounded-box flex items-center justify-center text-xl shrink-0 shadow-inner"
                ...
                <div>
                  <div class="flex items-center gap-2">
                    <h1 class="text-2xl font-bold text-base-content tracking-tight">{{ formData.name || 'Untitled Column' }}</h1>
                    <span v-if="column.isSystem" class="badge badge-neutral border-base-300 text-[10px] uppercase font-mono py-1">
                      System
                    </span>
                  </div>
                  <p class="text-xs text-base-content/60 font-mono mt-0.5">
                    ID: <span class="text-base-content/80">{{ column.id }}</span>
                  </p>
                </div>
              </div>

              <!-- Quick Live Preview Strip -->
              <div class="bg-base-100 border border-base-300 rounded-box p-3 shrink-0 flex flex-col gap-1.5 w-full md:w-64">
                <span class="text-[10px] text-base-content/50 uppercase font-mono tracking-wider">Table Header Preview</span>
                <div class="bg-base-200 border border-base-300 rounded px-2.5 py-1.5 flex items-center justify-between">
                  <div class="flex items-center gap-2 min-w-0">
                    <span class="w-2 h-2 rounded-full shrink-0" :style="{ backgroundColor: formData.color }"></span>
                    <span class="text-xs font-mono font-bold truncate text-base-content">{{ formData.name || 'Column' }}</span>
                  </div>
            ...
            <div class="card bg-base-200 border border-base-300 rounded-box p-6 space-y-6 shadow-sm">
              <div class="border-b border-base-300 pb-3">
                <h2 class="text-base font-bold text-base-content flex items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-primary"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/><path d="M9 21V9"/></svg>
                  General Information
                </h2>
                <p class="text-xs text-base-content/60">Define the column name, placeholder, and cell content behavior.</p>
              </div>
              ...
              <div class="space-y-4 pt-4 border-t border-base-300">
                <div class="pb-1">
                  <h3 class="text-sm font-bold text-base-content flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-secondary"><circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/></svg>
                    Color & Identification
                  </h3>
                  <p class="text-xs text-base-content/60">Choose a distinct color marker to easily spot this column in the breakdown table.</p>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3">
                  <button
                    ...
                    class="w-9 h-9 rounded-box transition-transform flex items-center justify-center shadow-md relative"
                    :style="{ backgroundColor: c }"
                    :class="formData.color === c ? 'scale-110 ring-2 ring-primary' : 'opacity-80 hover:opacity-100 hover:scale-105'"
                  >
                    <svg v-if="formData.color === c" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-primary-content drop-shadow"><polyline points="20 6 9 17 4 12"/></svg>
                  </button>

                  <!-- Custom Hex Input -->
                  <div class="flex items-center gap-2 ml-auto">
                    <span class="text-xs font-mono text-base-content/60">Custom:</span>
                    <input 
                      type="text" 
                      v-model="formData.color" 
                      class="input input-sm input-bordered w-28 bg-base-100 border-base-300 font-mono text-xs text-base-content" 
                      placeholder="#3b82f6"
                    />
                    <input 
                      type="color" 
                      v-model="formData.color" 
                      class="w-8 h-8 rounded border border-base-300 bg-transparent cursor-pointer" 
                    />
                  </div>
                </div>
              </div>
            ...
            <div class="card bg-base-200 border border-base-300 rounded-box p-6 space-y-6 shadow-sm">
              <div class="border-b border-base-300 pb-3">
                <h2 class="text-base font-bold text-base-content flex items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-secondary"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/></svg>
                  AI Assistant & Automation
                </h2>
                <p class="text-xs text-base-content/60">Configure default instructions used by the AI assistant when generating content for this column.</p>
              </div>
              ...
              <div class="space-y-4">
                <div class="space-y-2">
                  ...
                  <textarea 
                    ...
                    class="textarea textarea-bordered w-full bg-base-100 border-base-300 text-base-content text-sm focus:border-secondary font-mono leading-relaxed"
                  ></textarea>
                </div>
              </div>
            </div>
